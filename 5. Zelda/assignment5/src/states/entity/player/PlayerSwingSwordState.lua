...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if not entity.dead and entity:collides(self.swordHitbox) then
            entity:damage(1)
            ...
            if math.random(1) == 1  then
                ...
                heart_gameobject.onCollide = function()
                    if not heart_gameobject.collected then
                        heart_gameobject.collected = true
                        self.player:damage(-2)
                        gSounds['hit-player']:play()
                    end
                end

                table.insert(self.dungeon.currentRoom.objects, heart_gameobject)
            end
